...
static void find_obj(u8* argv0) {
  ...
  FATAL("Unable to find 'afl-compiler-rt.o' or 'afl-fish-pass.so'. Please set AFL_PATH");
...
static void edit_params(u32 argc, char** argv) {
  ...
  u8 fortify_set = 0, asan_set = 0, x_set = 0, bit_mode = 0, maybe_linking = 1;
  u8 *name;
  ...
  cc_params[cc_par_cnt++] = "-Xclang";
  cc_params[cc_par_cnt++] = alloc_printf("%s/afl-fish-pass.so", obj_path);
#endif /* ^USE_TRACE_PC */
  ...
  while (--argc) {
    ...
    if (!strncmp(cur, "-distance", 9)|| 
        ...
        !strncmp(cur, "-funcid", 7))
      cc_params[cc_par_cnt++] = "-mllvm";
    if (!strncmp(cur, "-debug", 6)) 
      is_debug = 1;
...
int main(int argc, char** argv) {
...
#ifdef USE_TRACE_PC
    SAYF(cCYA "afl-fish-fast [tpcg] " cBRI VERSION  cRST " by <kdsjzh@gmail.com>\n");
#else
    SAYF(cCYA "afl-fish-fast " cBRI VERSION  cRST " by <kdsjzh@gmail.com>\n");
#endif /* ^USE_TRACE_PC */
  ...
  if (argc < 2) {
    ...
    SAYF("\n"
         ...
         "  CC=%s/afl-fish-fast ./configure\n"
         "  CXX=%s/afl-fish-fast++ ./configure\n\n"
  ...
  //for (int i = 0; i < cc_par_cnt; i ++) printf("%s ", cc_params[i]); putchar('\n');
