...
// Prospector modified begin
std::map<std::uint32_t, std::uint32_t> func_priority;
std::map<std::string, std::uint32_t>   funcid_map;
// Prospector modified end
...
bool load_llvm_cfg(std::string const &fname, graph_llvm_t &graph_llvm) {
  ...
  bo::ref_property_map<graph_llvm_t *, std::string> gname(
      get_property(graph_llvm, bo::graph_name));
...
vertex_desc_cfg initialized_cfg_vertex(Vertex_llvm v, graph_cfg_t &cfg) {
  ...
  for (auto iter : boost::make_iterator_range(vertices(cfg))) {
    if (cfg[iter].name.compare(v.label) == 0) { return iter; }
  }
  ...
  std::map<std::string, std::string>::iterator f_iter =
      fcalls.find(cfg[vd].name);
  if (f_iter != fcalls.end()) {
    cfg[vd].is_fcall = true;
    cfg[vd].fname = f_iter->second;
    // cfg[vd].fname = f_iter->second.substr(0,
    // f_iter->second.find_last_of('.'));
  } else {
  ...
  if (cfg[vd].name.compare(first_bbname) == 0)
    cfg[vd].is_fstbb = true;
  else
    cfg[vd].is_fstbb = false;

  return vd;
}

void initialized_cfg_edge(Edge_cfg &e, Vertex_cfg &vsrc, Vertex_cfg &vdst,
                          bool is_single) {
...
void add_edge_to_cfg(Vertex_llvm src, Vertex_llvm dst, graph_cfg_t &cfg,
                     bool is_single) {
...
bool initialized_cfg_distance(graph_cfg_t &new_cfg) {
  ...
  std::vector<std::uint32_t> distances(bo::num_vertices(new_cfg),
                                       UNREACHABLE_DIST);
  // init_distances_from(cfg, fst_node, distances);
  // auto dist_pmap = bo::make_iterator_property_map(distances.begin(),
  // get(bo::vertex_index, new_cfg)); auto vis =
  // bo::make_bfs_visitor(bo::record_distances(dist_pmap, bo::on_tree_edge()));
  ...
  bo::dijkstra_shortest_paths(
      new_cfg, fst_node,
      boost::predecessor_map(&preds[0])
          .distance_map(
              boost::make_iterator_property_map(distances.begin(), idmap))
...
bool construct_cfg(std::string cfgname, std::string savename) {
  ...
  if (bo::num_vertices(cfg_graph_llvm) == 1) {
    ...
    cerr << "Empty graph with " << bo::num_vertices(cfg_graph_llvm)
         << std::endl;
    return true;
  }
  ...
  for (auto src : boost::make_iterator_range(vertices(cfg_graph_llvm))) {
    // auto src = *iter;
    // cout << cfg_graph_llvm[src].label << ", " << out_degree(src,
    // cfg_graph_llvm) << "\n";
    ...
    for (auto edge : make_iterator_range(bo::out_edges(src, cfg_graph_llvm))) {
      ...
      bool is_single =
          (bo::out_degree(src, cfg_graph_llvm) == 1) ? true : false;
      add_edge_to_cfg(cfg_graph_llvm[src], cfg_graph_llvm[dst], cfg_graph,
                      is_single);
    }
  }
...
std::uint32_t get_func_distance(graph_cfg_t &cfg, std::string fname) {
  ...
  for (auto src : boost::make_iterator_range(vertices(cfg))) {
    ...
    if (cfg[src].fname.find(fname + ",") == 0 ||
      ...
      if (cfg[src].distance < shortest) shortest = cfg[src].distance;
    }
  }
  return shortest;
}
...
vertex_desc_cg initialized_cg_vertex(std::string &fname, graph_cg_t &cg) {
  ...
  for (auto iter : boost::make_iterator_range(vertices(cg))) {
    if (cg[iter].name.compare(fname) == 0) { return iter; }
  }
  auto vd = bo::add_vertex(cg);
  cg[vd].name = fname;
  cg[vd].distance = 0;
  std::string cfg_file =
      temporary_dir + "/runtimes/cfg." + cg[vd].name + ".dot";
...
bool initialized_cg_edge(std::string &srcfname, std::string &dstfname,
                         graph_cg_t &cg) {
  // bool has_vd_src = if_cfg_exists(temporary_dir + "/runtimes/cfg." +
  // srcfname.substr(1, srcfname.length() - 2) + ".dot"),
  //      has_vd_dst = if_cfg_exists(temporary_dir + "/runtimes/cfg." +
  //      dstfname.substr(1, dstfname.length() - 2) + ".dot");
  // if (!has_vd_src) {
  ...
  if (std::find(functions.begin(), functions.end(), sfname) ==
          functions.end() ||
...
bool initialized_cg_distance(graph_cg_t &cg, std::string &prev_path,
                             graph_cg_t &new_cg) {
  std::string temp_path = prev_path + ".temp";
  ...
  std::vector<std::uint32_t> distances(bo::num_vertices(new_cg),
                                       UNREACHABLE_DIST);
  // init_distances_from(cfg, fst_node, distances);
  // auto dist_pmap = bo::make_iterator_property_map(distances.begin(),
  // get(bo::vertex_index, new_cfg)); auto vis =
...
bool construct_cg(std::string cgname, graph_cg_t &cg_graph,
                  graph_cg_t &new_cg) {
  ...
  for (auto src : boost::make_iterator_range(vertices(cg_graph_llvm))) {
    for (auto edge : make_iterator_range(bo::out_edges(src, cg_graph_llvm))) {
      ...
      initialized_cg_edge(cg_graph_llvm[src].label, cg_graph_llvm[dst].label,
                          cg_graph);
    }
  }
...
bool calc_and_save_map(std::string temporary_dir, graph_cg_t &cg_graph) {
  // std::ofstream dev_log(temporary_dir + "/dev_log.log",
  //                       std::ofstream::out | std::ofstream::app);
  // std::map<std::size_t, std::map<std::size_t, uint32_t>> shortest_func_dist;
  Json::Value shortest_func_dist, shortest_dst_to_src, shortest_func_dist_p,shortest_dst_to_src_p;
  for (auto vd_start : boost::make_iterator_range(vertices(cg_graph))) {
    std::vector<std::uint32_t> distances(bo::num_vertices(cg_graph),
                                         UNREACHABLE_DIST);
    ...
    // for (auto vert : bo::make_iterator_range(vertices(cg_graph))) {
    //   dev_log << vert << "," << cg_graph[vert].name << "\n";
    // }

    // Prospector modified end
    // take distances[vd] as distance
    // std::map<std::size_t, uint32_t> func_distance;
    Json::Value func_distance,func_distance_p;
    for (auto vd : boost::make_iterator_range(vertices(cg_graph))) {
      ...
      if (distances[vd] < UNREACHABLE_DIST / 2)
        if (!(distances[vd] == 0 && vd == vd_start)) {
          // don't store itself
          func_distance_p[std::to_string(vd)] =
              double(distances[vd]) / (func_priority[vd] + 1);
          func_distance[std::to_string(vd)] = double(distances[vd]);
          if (shortest_dst_to_src.isMember(std::to_string(vd))) {
            Json::Value func_dist = shortest_dst_to_src[std::to_string(vd)];
            
            func_dist[std::to_string(vd_start)] = double(distances[vd]);
          ...
          // priority-based distance
          if (shortest_dst_to_src_p.isMember(std::to_string(vd))) {
            Json::Value func_dist = shortest_dst_to_src_p[std::to_string(vd)];
            func_dist[std::to_string(vd_start)] =
                double(distances[vd]) / (func_priority[vd] + 1);
            //func_dist[std::to_string(vd_start)] = double(distances[vd]);
            shortest_dst_to_src_p[std::to_string(vd)] = func_dist;
          } else {
            Json::Value func_dist;
            func_dist[std::to_string(vd_start)] =
                double(distances[vd]) / (func_priority[vd] + 1);
    ...
    shortest_func_dist_p[std::to_string(vd_start)] = func_distance_p;
  }
  ...
  std::ofstream      distance_map_priority(temporary_dir + "/runtimes/callmap_p.json");
  Json::StyledWriter styledWriter;
  distance_map << styledWriter.write(shortest_func_dist);
  distance_map_priority << styledWriter.write(shortest_func_dist_p);
  distance_map.close();
  distance_map_priority.close();
  /* format dst_func: {src_func : distance} */
  std::ofstream      dst_to_src_map(temporary_dir + "/runtimes/calldst.json");
  std::ofstream      dst_to_src_map_priority(temporary_dir + "/runtimes/calldst_p.json");
  Json::StyledWriter styledWriter1;
  dst_to_src_map << styledWriter1.write(shortest_dst_to_src);
  dst_to_src_map.close();
  dst_to_src_map_priority << styledWriter1.write(shortest_dst_to_src_p);
  dst_to_src_map_priority.close();
  // dev_log.close();
  return true;
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    desc.add_options()("help,h", "produce help message")(
        "dot,d", po::value<std::string>()->required(),
        "Path to dot-file representing the "
        "control flow graph.")("temp,t", po::value<std::string>()->required(),
                               "Temporary folder contain dot "
                               "files.")("out,o",
                                         po::value<std::string>()->required(),
                                         "Path to out-file contain dot files "
                                         "with initial distance.")(
        "outmap,a", po::value<std::string>(),
        "Path to out-file contain shortest "
        "function distance.")("mode,m", po::value<std::string>()->required(),
                              "Run in cfg mode or cg mode.");
  ...
  if (mode == MODE_CFG) {
    ...
    if (bt.is_open()) {
      ...
      while (getline(bt, line)) {
        // std::size_t ppos = line.find(",");
        // std::string bbname = line.substr(0, ppos);
        // std::uint32_t priority = (int)(100 * atof(line.substr(ppos + 1,
        // line.length()).c_str()));
        if (bug_targets.find(line) == bug_targets.end())
          bug_targets.emplace(line, bug_id);

        // if (bug_priority.find(bug_id) == bug_priority.end())
        //   bug_priority.emplace(bug_id, priority);
    ...
    if (fb.is_open()) {
      std::size_t pos1 = cfg_file.find("cfg."), pos2 = cfg_file.find(".dot");
      ...
      while (getline(fb, line)) {
        ...
        std::size_t rpos = line.rfind(":"), lpos = line.find(":");
        std::string bbname_module = line.substr(lpos + 1, rpos - lpos - 1);
        if (bbname_module.compare(module_name) == 0) {
          cout << bbname_module << ", " << line << "\n";
          // first_bbname = line.substr(fstbb_pos + 1, line.length() -
          // fstbb_pos); for (auto src :
          // boost::make_iterator_range(vertices(test_cfg))) {
    ...
    for (auto vert : bo::make_iterator_range(vertices(new_cg))) {
      if (funcid_map.find(new_cg[vert].name) == funcid_map.end())
        funcid_map.emplace(new_cg[vert].name, vert);
    }
    /* read function priority from file*/
    std::ifstream fp(temporary_path + "/priority.csv");
    if (fp.is_open()) {
      std::string line;
      while (getline(fp, line)) {
        std::size_t   ppos = line.find(",");
        std::string   fname = line.substr(0, ppos);
        std::uint32_t priority =
            (int)(100 * atof(line.substr(ppos + 1, line.length()).c_str()));
        std::uint32_t func_id = funcid_map[fname];
        if (func_priority.find(func_id) == func_priority.end())
          func_priority.emplace(func_id, priority);
      }
    }
