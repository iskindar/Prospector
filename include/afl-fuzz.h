...
KHASH_SET_INIT_INT64(sequence_set)
KHASH_MAP_INIT_INT(target_byte_map, byteRange)
KHASH_MAP_INIT_INT(func_byte_score, uint32_t*)
...
struct queue_entry {
  ...
  bool trim_done,        /* Trimmed?                         */
      ...
      retry, passed_det, /* Deterministic stages passed?     */
  ...
  /* prospector structure */
  u8     *byte_score;  /* Probabaility to mutate a certain byte*/
  u32     align_len;   /* Extend input lentgh to 4-bytes data*/
  u32    *alias_table; /* Table for byte selection*/
  double *alias_prob;  /* Probability for bytes*/
  bool reach_focused_targets; /* deprecated:if reached focused targets */
  bool reach_focused_function;  /*deprecated: if reached focused function*/
#ifdef INTROSPECTION
  u32 bitsmap_size;
#endif
...
/* for prospector priority schedule*/
enum {
  /* 00 */ DISABLE_FUNC,
  /* 01 */ DISABLE_TARGET,
  /* 02 */ DISABLE_BYTE,
...
typedef struct afl_state {
  ...
  u8 *virgin_bits,           /* Regions yet untouched by fuzzing */
      *virgin_funcs,         /* Funcs yet untouched by fuzzing   */
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  struct queue_entry **top_rated, /* Top entries for bitmap bytes */
      **top_rated_explore, **top_rated_exploit;
  ...
  /* prospector structure */
  /* update reach counts based on different paths */
  khash_t(sequence_set) * reach_target_seq_set; /* a khash set containing
                                                   multiple target sequeneces */
              
  /* consturct target byte map */
  khash_t(target_byte_map) * target_byte_map;
  
  u8   use_byte_fitness;    /* use byte score to select bytes for mutation */
  //u8   use_byte_fitness_exploit;  /* collect byte fitness(range) for targets */
  u32 *reach_target_seqs;     /* record reached target sequence , only used in
                                 write_reach_log */
  u32 reach_target_seq_count; /* reached target sequence counter, only used in
                                 write_reach_log */
  u8 *last_reached_targets;   /* record the latest reached target for obtaining
                                 relative new reached targets */
  u8 *cur_reached_targets;   /* record the current reached targets for obtaining
                                relative new reached targets */
  u32 *focused_targets;       /* record the focused targets  */
  u32 focused_targets_count; /* count of the focused targets*/
  u32 remain_focused_targets_count; /* count of the remain focused targets*/
  u32 *focused_count;         /* count of a target marked as focused */
  int is_unique_target_seq;  /* indicate whether seed's target sequence is
                                unique*/
  u8 *byte_prob_norm_buf,    /* For ACO; normed probability of seeds */
      *byte_out_scratch_buf,  /* For ACO; kicked out of analysis queue during
                                creating alias table */
      *byte_in_scratch_buf;   /* For ACO */
  u32 aco_max_seed_len;      /* For ACO; max seed length */
  u32 *target_initial_priority; /* store target initial priority */
  u32 target_count;           /* count of target*/
  u8 has_new_paths; /* initialize before execution, assign has_new_bits to this variable */
  u32 *unique_reach_bits_count; /* unique reach bits count */
  u32 *reach_bits_count; /* total reach bits count */
  u8 reach_focused_targets; /* indicate whether execution hit focused targets*/
  u8 reach_focused_function; /* indicate whether execution reach focused function*/
  bool reach_new_targets; /* indicate whether execution reach new targets*/
  u8 disable_priority_choice; /* disable priority schedule */
  double max_target_priority; /* max dynamic target priority */
  double min_target_priority; /* min dynamic target priority */
  
  u64 last_reach_focused_time; /* last time reach focused targets */
  u64 last_func_focused_time;   /* last time reach focused functions*/
  u64 last_trigger_focused_time; /* last time trigger focused targets*/
  u32 valuable_function;
  u32 valuable_target;
  u64 last_update_focused_exec;
  u8 skip_intra_func;
  u64 last_update_focused_target_time; /* last time update focused targets (debug)*/

  /* fishfuzz structure */
  u32 *trigger_bits_count; 

  u32 current_func_covered, current_targets_reached, current_targets_triggered;

  u32 queued_retryed, queued_fuzzed_favored, queued_fuzzed_non_favored,
      queued_fuzzed_retryed;

  u64 last_reach_time, last_trigger_time, last_func_time, start_func_time,
      last_update_exec;

  u8 function_changed, target_changed, skip_inter_func, fish_seed_selection;
  ...
  /* fishfuzz debugging */
  u8 *fish_debug_log, /* debugging log file for fish fuzz */
      *exploit_debug_log, *cull_debug_log, *function_debug_log, *seed_selec_log,
      *exploit_log, *reach_log, *collection_log, *dev_log, *byte_score_log, *focused_targets_log,
      *target_priority_log;

  FILE *fish_debug_fd, *exploit_debug_fd, *cull_debug_fd, *function_debug_fd,
      *seed_selec_fd, *exploit_fd, *reach_fd, *collection_fd, *dev_fd,
      *byte_score_fd, *focused_targets_fd,*target_priority_fd;

  u64 last_log_time, log_cull_origin_time, log_cull_explore_time,
      log_cull_exploit_time, log_cull_other_time, log_update_explore_time;
...
void cull_queue_exploit_original(afl_state_t *);
void cull_queue_exploit(afl_state_t *);
...
/* Prospector function*/
void compare_targets(afl_state_t *afl, u8 *old_seed, u8 *new_seed, u32 len);
void update_last_reached_targets(afl_state_t *afl);
void collect_target_related_bytes(afl_state_t *afl, struct queue_entry *q,
                                  s32 byte_start, s32 byte_end);
void destroy_alias_buf(afl_state_t *afl);

void update_byte_score(afl_state_t *afl, struct queue_entry *q, u8 *seed_mem,
                       u8 *cur_input_mem, u32 cur_input_len);
u64 collect_target_byte_range(afl_state_t *afl, u64 prev_cksum , u8 byte_offset);
...
void expire_old_score(afl_state_t *afl, struct queue_entry *q);
void update_fitness_in_havoc(afl_state_t *afl ,struct queue_entry* q, u8* seed_mem, 
            u8* cur_input_mem, u32 cur_input_len);
...
// void write_collection_log(afl_state_t * afl); /* target-byte data collection
// */
/* FishFuzz runtimes */
#ifdef __cplusplus
extern "C" {
...
void initialized_dist_map(void);
...
void initialized_dist_map_p(void);
...
//void write_target_priority_log(afl_state_t *afl,std::vector<std::pair<double, std::uint32_t>> reached_bugs);
//void update_focused_targets(afl_state_t *afl);
#ifdef __cplusplus
}
#endif
