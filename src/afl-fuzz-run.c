...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  /* prospector*/
  if(!afl->focused_count) {
    q->reach_focused_targets = reach_focused_target(afl);
    q->reach_focused_function = reach_function(afl,afl->valuable_function);
  }
...
/* Prospector begin*/
// void update_last_reached_target(afl_state_t *afl, u8 *fish_map){
  
//   if(!afl->last_reached_target){
//     afl->last_reached_target = ck_alloc(sizeof(u8) * VMAP_COUNT / 8);
//   }
//   u8 *targets_map = fish_map + FUNC_SIZE;
//   compress_target(afl->last_reached_target, targets_map);
// }
/* Prospector end */
/* update fishfuzz states */
