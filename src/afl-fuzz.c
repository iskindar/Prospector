...
static void usage(u8 *argv0, int more_help) {
  ...
  SAYF(
      ...
      "  -P priority   - disable priority: \n"
      "                  func,target,byte,none ,default: none."
      "coe, lin\n"
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  afl->use_byte_fitness = 1; /*prospector option:use byte fitness to select bytes at exploit stage*/
  afl->disable_priority_choice = DISABLE_NONE; // default priority schedule : disable none
  while (
      (opt = getopt(
           argc, argv,
           "+Ab:B:c:CdDe:E:hi:I:f:F:g:G:l:L:m:M:nNOo:p:P:RQs:S:t:T:UV:WXx:YZ")) >
      0) {
    ...
    switch (opt) {
      ...
      case 'p':                                           /* Power schedule */
        ...
        case 'P':                                           /* Priority schedule */

        if (!stricmp(optarg, "func")) {

          afl->disable_priority_choice = DISABLE_FUNC;

        } else if (!stricmp(optarg, "target")) {

          afl->disable_priority_choice = DISABLE_TARGET;

        } else if (!stricmp(optarg, "byte")) {

          afl->disable_priority_choice = DISABLE_BYTE;

        } else if (!stricmp(optarg, "none")) {

          afl->disable_priority_choice = DISABLE_NONE;

        } 
        else {

          FATAL("Unknown -P priority schedule");
  ...
  if(afl->disable_priority_choice == DISABLE_FUNC){
    initialized_dist_map();
  }else{
    initialized_dist_map_p();
  }
  ACTF("Loading the target2function map...");
  initialized_target_function_map();
  afl->fish_seed_selection = INTER_FUNC_EXPLORE;
  if(afl->disable_priority_choice == DISABLE_BYTE){
    afl->use_byte_fitness = 0;
  }else{
    afl->use_byte_fitness = 1;
  }

  /* Prospector initialization */
  if(unlikely(!afl->target_initial_priority)){
    afl->target_initial_priority = ck_alloc(sizeof(u32) * VMAP_COUNT);
    afl->target_count = 0;  
  }
  if (unlikely(!afl->focused_targets)) {
    afl->focused_targets =
        ck_alloc(sizeof(u32) * VMAP_COUNT );  
    afl->focused_targets_count =0 ;
    afl->focused_count = ck_alloc(sizeof(u32) * VMAP_COUNT);
  }
  afl->valuable_target = 0;
  afl->reach_focused_function = 0;
  afl->last_update_focused_exec = 0;
   ACTF("Loading the target priority map...");
  initialized_target_priority_map(afl);
  add_target_group_into_focused(afl); // initialize the focused target with initial priority
  
  afl->last_update_focused_target_time = get_cur_time();
  OKF("The total count of targets is %d", afl->target_count);
  OKF("The focused count of targets is %d", afl->focused_targets_count);
  afl->max_target_priority = 0;
  afl->min_target_priority = 0;
...
stop_fuzzing:
  ...
  if(afl->focused_targets_fd) fclose(afl->focused_targets_fd);
  if(afl->byte_score_fd) fclose(afl->byte_score_fd);
  ...
  ck_free(afl->sync_id);
  /*fishfuzz free*/
  ck_free(afl->reach_bits_count);
  ck_free(afl->trigger_bits_count);
  /* Prospector free*/
  ck_free(afl->focused_targets);
  ck_free(afl->focused_count);
  ck_free(afl->target_initial_priority);
  ck_free(afl->unique_reach_bits_count);
  //ck_free(afl->cur_reached_targets);
