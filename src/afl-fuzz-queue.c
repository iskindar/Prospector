...
void update_function_cov(afl_state_t *afl, char *fish_map) {
  if (unlikely(!afl->virgin_funcs)) {
    afl->virgin_funcs = ck_alloc(sizeof(char) * FUNC_SIZE);
  }
  ...
  for (u32 j = 0; j < reachable_count; j++) {
    if (fish_map[reachable_funcs[j]] && afl->has_new_paths) {
      afl->last_func_focused_time = get_cur_time();
    }
  }
}
...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  q->reach_focused_function = 0;
  q->reach_focused_targets = 0;
  ...
  if (afl->shm.fishfuzz_mode && afl->shm.fish_map) {
    update_function_cov(afl, afl->shm.fish_map);
...
void destroy_queue(afl_state_t *afl) {
  ...
  for (i = 0; i < afl->queued_items; i++) {
    ...
    ck_free(q->trace_target);
    ck_free(q->trace_func);
...
void update_bitmap_score_origin(afl_state_t *afl, struct queue_entry *q) {
  u32 i;
  u64 fav_factor;
  u64 fuzz_p2;
...
void update_bitmap_score_target(afl_state_t *afl, struct queue_entry *q) {
  u32 i;
  u64 fav_factor = q->exec_us * q->len;

  if (!q->trace_target || !afl->top_rated_exploit) return;

  /* For every byte set in afl->fsrv.trace_bits[], see if there is a previous
     winner, and how it compares to us. */
  for (i = 0; i < VMAP_COUNT; ++i) {
    /* if target i is reached */
    if (q->trace_target[i >> 3] & (1 << (i & 0x7))) {
      if (afl->top_rated_exploit[i]) {
        /* Faster-executing or smaller test cases are favored. */
        if (fav_factor >
            afl->top_rated_exploit[i]->exec_us * afl->top_rated_exploit[i]->len)
          continue;
      }
...
void update_bitmap_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  if (afl->shm.fishfuzz_mode && afl->shm.fish_map)
    update_bitmap_score_target(afl, q);
}
...
void cull_queue_origin(afl_state_t *afl) {
  ...
  if (!afl->pending_favored)
    afl->skip_intra_func = 1; /*prospector modification*/
  for (i = 0; i < afl->queued_items; i++) {
...
void cull_queue_explore(afl_state_t *afl) {
  if (likely(!afl->function_changed || afl->non_instrumented_mode)) { return; }

  u32 i;

  afl->function_changed = 0;
...
/*Prospector begin*/ 
  for (i = 0; i < FUNC_SIZE; i++) {
    ...
    if (afl->top_rated_explore[i] &&
        (!afl->virgin_funcs[i] || i == afl->valuable_function)) {
...
/* Prospector end */ 
  if (!afl->pending_favored) afl->skip_inter_func = 1;
...
void cull_queue_exploit_original(afl_state_t *afl) {
#define TRACE_MINI_VISITED(trace_mini, idx) \
  (trace_mini[idx >> 3] & (1 << (idx & 7)))
  ...
  if (afl->non_instrumented_mode || (!afl->target_changed && !should_skip))
    return;
  ...
  for (i = 0; i < VMAP_COUNT; i++) {
    if (temp_v[i >> 3] & (1 << (i & 7)) && afl->reach_bits_count[i] &&
      ...
      while (k--)
        if (selected->trace_target[k]) temp_v[k] &= ~selected->trace_target[k];
  ...
  u64 avg_violation_trigger = 0, avg_violation_visit = 0;
  if (afl->current_targets_triggered)
    avg_violation_trigger = total_trigger_cnt / afl->current_targets_triggered;
  if (afl->current_targets_reached)
    avg_violation_visit = total_visit_cnt / afl->current_targets_reached;
      ...
      if (avg_violation_visit) {
        for (i = 0; i < VMAP_COUNT; i++) {
          if (afl->reach_bits_count[i] && !afl->trigger_bits_count[i] &&
             ...
             */
            if (!selected->favored || !selected->was_fuzzed || selected->retry)
              continue;

            if (avg_violation_visit > afl->reach_bits_count[i] &&
                rand_below(afl, 100) >= rate) {
              afl->queued_retryed++;
              selected->retry = 1;
            }
          }
...
void cull_queue_exploit(afl_state_t *afl) {
#define TRACE_MINI_VISITED(trace_mini, idx) \
  (trace_mini[idx >> 3] & (1 << (idx & 7)))
  ...
  // u32 total_visit_cnt = 0, total_n_bugs = 0;
  for (i = 0; i < VMAP_COUNT; i++) {
    if (temp_v[i >> 3] & (1 << (i & 7)) && afl->unique_reach_bits_count[i] &&
        get_funcid(i) == afl->valuable_function &&
        !afl->trigger_bits_count[i]) {
      struct queue_entry *selected = afl->top_rated_exploit[i];
  ...
  if (afl->queued_favored) {
    ...
    if (rate < 50) {
      ...
      if (avg_violation_visit) {
        for (i = 0; i < VMAP_COUNT; i++) {
          /*Prospector begin*/ 
          if (afl->unique_reach_bits_count[i] && !afl->trigger_bits_count[i] &&
              get_funcid(i) == afl->valuable_function) {
          /*Prospector end*/ 
            struct queue_entry *selected = afl->top_rated_exploit[i];
...
void cull_queue(afl_state_t *afl) {
  ...
  /* we only update if we have enough execs since last execs or first 30 mins */
  bool has_enough_execs =
      afl->fsrv.total_execs - afl->last_update_focused_exec >=
      MINIMAL_UPDATE_EXEC;

  if (!afl->last_update_exec ||
  ...
  if (afl->fish_seed_selection == INTER_FUNC_EXPLORE) {
    if (ratio_of_reachable_func(afl) > random_value) {
      // if (should_skip) afl->skip_inter_func = 1;
      //  no focused target reached at a short time
      if (ratio_of_reached_focused_target(afl) < random_value) {
        afl->score_changed = 1;
      ...
      if (ratio_of_time(afl, afl->last_func_focused_time) > random_value &&
          has_enough_execs) {
        write_focused_targets_log(afl);

        if (afl->disable_priority_choice == DISABLE_TARGET) {
          target_ranking_original(afl);
        } else {
          target_ranking(afl);
        }

        update_focused_targets(afl);  // prospector modification
        afl->last_update_focused_exec = afl->fsrv.total_execs;
        afl->last_func_focused_time = get_cur_time();
        afl->start_func_time = get_cur_time();
        write_fishfuzz_log(afl, INTER_FUNC_EXPLORE, INTER_FUNC_EXPLORE);
      }
    }

  } else if (afl->fish_seed_selection == INTRA_FUNC_EXPLORE) {
    if (ratio_of_time(afl, afl->last_reach_focused_time) > random_value && has_enough_execs) {
      if (ratio_of_reached_focused_target(afl) < random_value 
      ) {
        write_focused_targets_log(afl);
        if (afl->disable_priority_choice == DISABLE_TARGET) {
    ...
    if ((ratio_of_time(afl, afl->last_trigger_focused_time) > random_value ||
         (!afl->focused_targets_count &&
          ratio_of_triggered_focused_target(afl) > random_value)) &&
        has_enough_execs) {
      write_focused_targets_log(afl);
      if (afl->disable_priority_choice == DISABLE_TARGET) {
        target_ranking_original(afl);
  ...
  switch (afl->fish_seed_selection) {
    case INTRA_FUNC_EXPLORE:
      ...
      afl->log_cull_origin_time += (get_cur_time() - tmp_time_log);
      return;
    case INTER_FUNC_EXPLORE:
      cull_queue_explore(afl);
      afl->log_cull_explore_time += (get_cur_time() - tmp_time_log);
      return;
    case TARGET_EXPLOIT:
      if (afl->disable_priority_choice == DISABLE_TARGET)
        cull_queue_exploit_original(afl);
      else
        cull_queue_exploit(afl);
      afl->log_cull_exploit_time += (get_cur_time() - tmp_time_log);
      return;
    default:
