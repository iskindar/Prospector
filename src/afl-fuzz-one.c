...
/* Helper function to verify if determine stage takes too long (e.g. > 30 min)
 */
static u8 is_too_large(afl_state_t *afl, struct queue_entry *q) {
  u64 det_power = 0;
  ...
  /* if determine stage is expected to last for > 30 mins.
     if there are less selected bytes in eff_map, means more
     bytes will be skipped in determine stage, given higher
     limit. only works after 1h. */

  if (!afl->blocks_eff_select) return 0;

  u64 eff_limit =
      SKIP_DET_US_TLIMIT * afl->blocks_eff_total / afl->blocks_eff_select;

  if (det_power * q->exec_us > eff_limit &&
      get_cur_time() - afl->start_time > (1000U * 3600)) {
    return 1;
  }
...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  u32 a_len = 0;
  u32 third1_stage, third2_stage;  // time to update byte alias table
#ifdef IGNORE_FINDS
  ...
  if (likely(afl->pending_favored + afl->queued_retryed)) {
    /* If we have any favored, non-fuzzed new arrivals in the queue,
       possibly skip to them at the expense of already-fuzzed or non-favored
       cases. */

    if (afl->fish_seed_selection != INTRA_FUNC_EXPLORE) {
  ...
  if (unlikely(!afl->non_instrumented_mode && !afl->queue_cur->trim_done &&
    ...
    if (len != afl->queue_cur->len) {
      ...
      if (afl->queue_cur->len % ACO_GROUP_SIZE) {
        afl->queue_cur->align_len = afl->queue_cur->len -
                                    afl->queue_cur->len % ACO_GROUP_SIZE +
                                    ACO_GROUP_SIZE;
      } else {
        afl->queue_cur->align_len = afl->queue_cur->len;
      }
    ...
    if (afl->use_byte_fitness) {
      if (unlikely(!afl->queue_cur->byte_score)) {
        afl->queue_cur->byte_score =
            ck_alloc(sizeof(u32) * afl->queue_cur->align_len);
        // initialize byte score as zero
        memset(afl->queue_cur->byte_score, INIT_BYTE_SCORE, afl->queue_cur->align_len);
      }

      if (unlikely(!afl->queue_cur->alias_table)) {
        afl->queue_cur->alias_table =
            ck_alloc(sizeof(u32) * afl->queue_cur->len);
        afl->queue_cur->alias_prob =
            ck_alloc(sizeof(double) * afl->queue_cur->len);
      }
    }
    /* prospector end*/
  ...
  /* skip too large seeds */
  if (get_pos_length(afl->queued_items * 0.9) < afl->queue_cur->len &&
      get_pos_length(afl->queued_items * 0.9) &&
      is_too_large(afl, afl->queue_cur)) {
    goto custom_mutator_stage;
  }
...
#define FLIP_BIT(_ar, _b)                   \
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (common_fuzz_stuff(afl, out_buf, len)) { goto abandon_entry; }
    // prev_target_cksum = collect_target_byte_range(afl, prev_target_cksum, 0);
    if(afl->use_byte_fitness) cal_init_seed_score(afl, 0);
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l)-1) - EFF_APOS(_p) + 1)
  ...
  for (i = 0; i < len - 1; ++i) {
    ...
    *(u16 *)(out_buf + i) ^= 0xFFFF;
    ...
    if (common_fuzz_stuff(afl, out_buf, len)) { goto abandon_entry; }
    //prev_target_cksum = collect_target_byte_range(afl, prev_target_cksum, 1);
    if(afl->use_byte_fitness) cal_init_seed_score(afl, 1);
    ++afl->stage_cur;
  ...
  for (i = 0; i < len - 3; ++i) {
    ...
    *(u32 *)(out_buf + i) ^= 0xFFFFFFFF;
    ...
    if (common_fuzz_stuff(afl, out_buf, len)) { goto abandon_entry; }
    //prev_target_cksum = collect_target_byte_range(afl, prev_target_cksum, 3);
    if(afl->use_byte_fitness) cal_init_seed_score(afl, 3);
    ++afl->stage_cur;
    ...
    for (j = 1; j <= ARITH_MAX; ++j) {
      ...
      if ((orig & 0xff) + j > 0xff && !could_be_bitflip(r1)) {
        ...
        *(u16 *)(out_buf + i) = orig + j;
        ...
        if (common_fuzz_stuff(afl, out_buf, len)) { goto abandon_entry; }
        // prev_target_cksum =
        //     collect_target_byte_range(afl, prev_target_cksum, 1);
        if(afl->use_byte_fitness) cal_init_seed_score(afl, 1);
        ++afl->stage_cur;
    ...
    for (j = 1; j <= ARITH_MAX; ++j) {
      ...
      if ((orig & 0xffff) + j > 0xffff && !could_be_bitflip(r1)) {
        ...
        *(u32 *)(out_buf + i) = orig + j;
        ...
        if (common_fuzz_stuff(afl, out_buf, len)) { goto abandon_entry; }
        // prev_target_cksum =
        //     collect_target_byte_range(afl, prev_target_cksum, 3);
        if(afl->use_byte_fitness) cal_init_seed_score(afl, 3);
        ++afl->stage_cur;
...
#define MUTATE_ASCII_DICT 64
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    /* prospector modified*/
    if (afl->use_byte_fitness) {
      if (afl->stage_cur == third1_stage)
        create_byte_alias_table(afl, afl->queue_cur);
      if (afl->stage_cur == third2_stage)
        create_byte_alias_table(afl, afl->queue_cur);
    }
    ...
    for (i = 0; i < use_stacking; ++i) {
      ...
      switch ((r = rand_below(afl, r_max))) {
...
#endif
          FLIP_BIT(out_buf, URfitness(afl, temp_len << 3));
          break;
        }
        ...
        case 4 ... 7: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          out_buf[URfitness(afl, temp_len)] =
        ...
        case 10 ... 11: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          *(u16 *)(out_buf + URfitness(afl, temp_len - 1)) = SWAP16(
              interesting_16[rand_below(afl, sizeof(interesting_16) >> 1)]);
        ...
        case 14 ... 15: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          *(u32 *)(out_buf + URfitness(afl, temp_len - 3)) = SWAP32(
              interesting_32[rand_below(afl, sizeof(interesting_32) >> 2)]);
        ...
        case 16 ... 19: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          out_buf[URfitness(afl, temp_len)] -= 1 + rand_below(afl, ARITH_MAX);
          break;
        }
        ...
        case 40 ... 43: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          out_buf[URfitness(afl, temp_len)] ^= 1 + rand_below(afl, 255);
          break;
        }
        ...
        case 54: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          out_buf[URfitness(afl, temp_len)] ^= 0xff;
          break;
        }
        ...
        default:
          ...
          if (afl->extras_cnt) {
            if (r < 2) {
              ...
              u32 insert_at = URfitness(afl, temp_len - extra_len + 1);
#ifdef INTROSPECTION
              ...
              u32 insert_at = URfitness(afl, temp_len + 1);
#ifdef INTROSPECTION
    ...
    if(afl->use_byte_fitness){
      update_fitness_in_havoc(afl,afl->queue_cur, orig_in,out_buf,temp_len);
      expire_old_score(afl,afl->queue_cur);
      //write_byte_score_log(afl);
    }
...
abandon_entry:
  ...
  if (!afl->stop_soon && !afl->queue_cur->cal_failed &&
    ...
    if (afl->queue_cur->favored) {
      --afl->pending_favored;
      ++afl->queued_fuzzed_favored;
    } else
      ++afl->queued_fuzzed_non_favored;
  }
  if (afl->queue_cur->retry) ++afl->queued_fuzzed_retryed;
